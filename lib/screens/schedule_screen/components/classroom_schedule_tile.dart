...
import 'package:zst_schedule/screens/schedule_screen/components/classroom_lesson_dialog.dart';
import 'package:zst_schedule/string_extencions.dart';

class ClassroomScheduleTile extends StatelessWidget {
  const ClassroomScheduleTile({
    Key? key,
    required this.hours,
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      subtitle: Text((() {
        String _text = '';
        if (lesson[0].schoolClass!.fullName != null) {
          _text +=
              "${lesson[0].schoolClass!.fullName} (${lesson[0].schoolClass!.code})";
        } else {
          _text += lesson[0].schoolClass!.code;
        }
        _text += ", ";
